...
export class AutoSaveSystem {
    constructor(options = {}) {
        ...
        // Initialize default serializers and deserializers
        this.initializeDefaultSerializers();
        this.initializeDefaultDeserializers();
    ...
    initializeDefaultSerializers() {
        ...
        this.addSerializer('dwarf', (dwarf) => {
            return {
                ...
                // Death state
                isDead: dwarf.isDead,
                deathTime: dwarf.deathTime,
                deathCause: dwarf.deathCause,
        ...
        // Lifespan system serializer
        this.addSerializer('lifespanSystem', (lifespanSystem) => {
            if (!lifespanSystem) return null;
            
            return {
                skulls: lifespanSystem.skulls || [],
                stats: lifespanSystem.stats || {
                    totalDeaths: 0,
                    deathsFromAge: 0,
                    deathsFromCombat: 0,
                    combatKills: 0,
                ...
                config: {
                    maxLifespan: lifespanSystem.config?.maxLifespan,
                    minLifespan: lifespanSystem.config?.minLifespan,
                    agingStartAge: lifespanSystem.config?.agingStartAge,
                    maxHealth: lifespanSystem.config?.maxHealth,
                    healingRate: lifespanSystem.config?.healingRate,
                    baseDamage: lifespanSystem.config?.baseDamage,
                    damageVariance: lifespanSystem.config?.damageVariance,
                    criticalChance: lifespanSystem.config?.criticalChance,
                    attackRange: lifespanSystem.config?.attackRange,
                    territorialCombatChance: lifespanSystem.config?.territorialCombatChance
                }
            };
        });
    }
    ...
    /**
     * Initialize default deserializers for common game objects
     */
    initializeDefaultDeserializers() {
        // Dwarf deserializer
        this.addDeserializer('dwarf', (data, gameRef) => {
            // Create basic dwarf object (assuming Dwarf constructor exists)
            let dwarf;
            if (typeof window !== 'undefined' && window.Dwarf) {
                dwarf = new window.Dwarf(data.x, data.y, data.name, data.isAdult);
            } else {
            ...
            // Initialize lifespan system data if not present
            if (!dwarf.maxLifespan && gameRef && gameRef.lifespanIntegration) {
                gameRef.lifespanIntegration.initializeDwarf(dwarf);
            }
            
            // Restore combat references after a delay (to allow all dwarfs to be loaded)
            if (data.combatTarget && gameRef) {
                setTimeout(() => {
                    if (gameRef.dwarfs) {
        ...
        // Lifespan system deserializer
        this.addDeserializer('lifespanSystem', (data, gameRef) => {
            if (!data || !gameRef || !gameRef.lifespanIntegration) return null;
            
            const lifespanSystem = gameRef.lifespanIntegration.lifespanSystem;
            
            // Restore skulls
            if (data.skulls) {
                lifespanSystem.skulls = data.skulls;
            }
            
            // Restore statistics
            if (data.stats) {
                Object.assign(lifespanSystem.stats, data.stats);
            }
            
            // Restore configuration if needed
            if (data.config) {
                Object.assign(lifespanSystem.config, data.config);
            }
    ...
    collectSaveData() {
        ...
        // Serialize lifespan system state
        if (game.lifespanIntegration && game.lifespanIntegration.lifespanSystem) {
            const lifespanSerializer = this.serializers.get('lifespanSystem');
            if (lifespanSerializer) {
                saveData.lifespanSystem = lifespanSerializer(game.lifespanIntegration.lifespanSystem);
            }
        }
    ...
    /**
     * Restore saved data to the game
     * @param {Object} saveData - Save data to restore
     * @param {Object} gameRef - Game reference for restoration
     * @returns {boolean} Success status
     */
    restoreSaveData(saveData, gameRef) {
        try {
            // Restore game state
            if (saveData.gameState && gameRef) {
                Object.assign(gameRef, saveData.gameState);
            }
            
            // Restore dwarfs
            if (saveData.dwarfs && Array.isArray(saveData.dwarfs)) {
                const dwarfDeserializer = this.deserializers.get('dwarf');
                if (dwarfDeserializer && gameRef) {
                    gameRef.dwarfs = saveData.dwarfs.map(dwarfData => 
                        dwarfDeserializer(dwarfData, gameRef)
                    );
                }
            }
            ...
            if (saveData.buildings && Array.isArray(saveData.buildings)) {
                // Buildings typically don't need complex deserialization
                if (gameRef) {
                    gameRef.buildings = saveData.buildings.map(buildingData => ({
                        ...buildingData
                    }));
                }
            }
            
            // Restore lifespan system
            if (saveData.lifespanSystem && gameRef && gameRef.lifespanIntegration) {
                const lifespanDeserializer = this.deserializers.get('lifespanSystem');
                if (lifespanDeserializer) {
                    lifespanDeserializer(saveData.lifespanSystem, gameRef);
                }
            }
            
            // Restore reproduction system statistics (read-only)
            if (saveData.reproductionStats && gameRef && gameRef.reproductionSystem) {
                // Statistics are typically recalculated rather than restored
                console.log('AutoSaveSystem: Reproduction statistics loaded (reference only)');
            }
            
            console.log('AutoSaveSystem: Save data restored successfully');
            return true;
            
        } catch (error) {
            console.error('AutoSaveSystem: Error restoring save data:', error);
            return false;
        }
    }
